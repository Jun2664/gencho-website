...
**重要**: Claude Codeはこのプロジェクトで作業する際、必ず日本語で返答してください。

## FTP認証完了の記録

- **日付**: 2025-09-30
- **状態**: FTPパスワードを修正済み、デプロイ再テスト準備完了

## プロジェクト概要

Gencho Websiteは、WordPress連携と自動デプロイ機能を搭載したNext.jsベースのコーポレートサイトです。静的サイト生成を使用し、コード変更またはWordPressコンテンツ更新時に自動的にデプロイされます。

**主要機能**: ルートページ (`/`) には、Webスクレイピングスクリプトで作成されたmonjusou.comの完全なクローンが表示されます。
...
### 変更を加える場合
1. `src/`内のコードを編集
2. `npm run dev`でローカルテスト
3. `npm run build`でビルド
4. mainブランチへpush → 自動デプロイ
